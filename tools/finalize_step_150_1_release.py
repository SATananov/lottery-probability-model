...
CLEAN_MANIFEST = ROOT / "CLEAN_ZIP_MANIFEST_STEP150_1.md"
FULL_MANIFEST = ROOT / "FULL_CLEAN_CHECKPOINT_MANIFEST_STEP150_1.md"
METADATA_FILES = (RELEASE_MANIFEST, CLEAN_MANIFEST, FULL_MANIFEST)
CHECKPOINT = "Step 150.1"
CHECKPOINT_TITLE = "Deep Dynamic UI Localization & User-Friendly Table Repair"
BASE_COMMIT = "a1dc3e5"
...
def write_manifests() -> dict:
    ...
    release = {
        ...
        "ui_language_integrity": {
            ...
            "deep_dynamic_audit": "src/v150_1_deep_ui_integrity_engine.py",
            "static_ui_literal_rows": status.get("static_ui_literal_rows"),
            "static_ui_literal_failures": status.get("static_ui_literal_failures"),
            "unique_csv_headers": status.get("unique_csv_headers"),
            "table_header_failures": status.get("table_header_failures"),
            "unique_dynamic_values": status.get("unique_dynamic_values"),
            "user_facing_dynamic_failures": status.get("user_facing_dynamic_failures"),
    ...
    clean = f"""# CLEAN ZIP MANIFEST — STEP 150.1

- Project: `lottery-probability-model`
- Checkpoint: `Step 150.1 — {CHECKPOINT_TITLE}`
...
- Step 150.1 verification: `STEP_150_1_VERIFY_OK`
- Table-header failures: **{status.get('table_header_failures', 'unknown')}**
- User-facing dynamic-text failures: **{status.get('user_facing_dynamic_failures', 'unknown')}**
...
- Dynamic JSON/CSV values and nested JSON output are localized at runtime.
- Every discovered CSV table header is audited and localized.
- Missing values and internal method names are rendered as user-friendly Bulgarian.
- Internal IDs, SHA-256 signatures, paths and UTC columns are hidden by default.
- Long metric values wrap instead of being shortened with ellipses.
...
"""
    ...
    full = rf"""# Full CLEAN Checkpoint Manifest — Step 150.1
...
- Static visible-text rows: `{status.get('static_ui_literal_rows', 'unknown')}`
- Static visible-text failures: `{status.get('static_ui_literal_failures', 'unknown')}`
- Unique table headers: `{status.get('unique_csv_headers', 'unknown')}`
- Table-header failures: `{status.get('table_header_failures', 'unknown')}`
- Unique dynamic values: `{status.get('unique_dynamic_values', 'unknown')}`
- User-facing dynamic failures: `{status.get('user_facing_dynamic_failures', 'unknown')}`
- Screenshot regression failures: `{status.get('screenshot_regression_failures', 'unknown')}`
...
def default_archive_path() -> Path:
    ...
    return ROOT.parent / f"lottery-probability-model_STEP150_1_DEEP-DYNAMIC-UI-LOCALIZATION-USER-FRIENDLY-TABLE-REPAIR_FULL-CLEAN_{stamp}.zip"


def main() -> int:
    parser = argparse.ArgumentParser(description="Regenerate, verify or package Step 150.1 release metadata")
