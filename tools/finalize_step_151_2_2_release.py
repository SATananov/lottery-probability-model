...
from src.v151_2_1_user_backup_ui_integrity_engine import audit_user_backup_ui, write_artifacts as write_step151_2_1_artifacts
from src.v151_2_2_runtime_import_compatibility_engine import (
    CHECKPOINT,
    CHECKPOINT_TITLE,
    audit_runtime_import_compatibility,
    write_artifacts as write_step151_2_2_artifacts,
)

RELEASE_MANIFEST = ROOT / "release-manifest.json"
CLEAN_MANIFEST = ROOT / "CLEAN_ZIP_MANIFEST_STEP151_2_2.md"
FULL_MANIFEST = ROOT / "FULL_CLEAN_CHECKPOINT_MANIFEST_STEP151_2_2.md"
...
def write_manifests() -> dict:
    ...
    write_step151_2_2_artifacts()

    repository = audit_repository_sync()
    backup_ui = audit_user_backup_ui()
    runtime_import = audit_runtime_import_compatibility()
    if not repository.get("ok") or not backup_ui.get("ok") or not runtime_import.get("ok"):
        return {
            "ok": False,
    ...
    release = {
        ...
        "runtime_import_compatibility": runtime_import,
        "fresh_clone_verification_command": "python scripts/verify_step_151_2_2.py --require-synced",
    ...
    CLEAN_MANIFEST.write_text(
        f"""# CLEAN ZIP MANIFEST — STEP 151.2.2
...
The archive is push-ready. Final synchronization is accepted only after `git push origin main` and `python scripts/verify_step_151_2_2.py --require-synced` pass on the operator machine.
""",
        encoding="utf-8",
        newline="\n",
    )
    ...
    FULL_MANIFEST.write_text(
        f"""# Full CLEAN Checkpoint Manifest — Step 151.2.2
...
def default_archive_path() -> Path:
    ...
    return ROOT.parent / f"lottery-probability-model_STEP151_2_2_RUNTIME-IMPORT-COMPATIBILITY_FULL-CLEAN_{stamp}.zip"
