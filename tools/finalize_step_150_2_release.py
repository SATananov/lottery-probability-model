...
CLEAN_MANIFEST = ROOT / "CLEAN_ZIP_MANIFEST_STEP150_2.md"
FULL_MANIFEST = ROOT / "FULL_CLEAN_CHECKPOINT_MANIFEST_STEP150_2.md"
METADATA_FILES = (RELEASE_MANIFEST, CLEAN_MANIFEST, FULL_MANIFEST)
CHECKPOINT = "Step 150.2"
CHECKPOINT_TITLE = "Plain Bulgarian User Language & Complete Technical Separation"
BASE_COMMIT = "6007b31"
...
def write_manifests() -> dict:
    ...
    release = {
        ...
        "ui_language_integrity": {
            ...
            "plain_language_layer": "src/v150_2_plain_language.py",
            "plain_language_audit": "src/v150_2_plain_language_integrity_engine.py",
            "broad_static_literal_rows": status.get("broad_static_literal_rows"),
            "active_ui_literal_rows": status.get("active_ui_literal_rows"),
            "active_ui_literal_failures": status.get("active_ui_literal_failures"),
            "unique_dynamic_keys_and_headers": status.get("unique_dynamic_keys_and_headers"),
            "dynamic_key_failures": status.get("dynamic_key_failures"),
            "screenshot_regression_failures": status.get("screenshot_regression_failures"),
            "utf8_files_checked": status.get("utf8_files_checked"),
            "utf8_failures": status.get("utf8_failures"),
    ...
    CLEAN_MANIFEST.write_text(
        f"""# CLEAN ZIP MANIFEST — STEP 150.2

- Project: `lottery-probability-model`
- Checkpoint: `Step 150.2 — {CHECKPOINT_TITLE}`
...
- Step 150.2 verification: `STEP_150_2_VERIFY_OK`
- Active UI literal failures: **{status.get('active_ui_literal_failures', 'unknown')}**
- Dynamic key/header failures: **{status.get('dynamic_key_failures', 'unknown')}**
- Screenshot regression failures: **{status.get('screenshot_regression_failures', 'unknown')}**
- UTF-8 failures: **{status.get('utf8_failures', 'unknown')}**
...
- Mixed Bulgarian-English decision criteria are replaced by complete Bulgarian sentences.
- Research tables show concise user summaries in normal mode.
- Raw statistics, identifiers, paths and signatures are separated into technical sections.
- Unknown dynamic English prose cannot leak into Bulgarian user mode.
- Active UI files pass strict UTF-8 and Cyrillic integrity checks.
...
""",
    ...
    FULL_MANIFEST.write_text(
        f"""# Full CLEAN Checkpoint Manifest — Step 150.2
...
- Broad static literal rows: `{status.get('broad_static_literal_rows', 'unknown')}`
- Active UI literals: `{status.get('active_ui_literal_rows', 'unknown')}`
- Active UI literal failures: `{status.get('active_ui_literal_failures', 'unknown')}`
- Dynamic keys and headers: `{status.get('unique_dynamic_keys_and_headers', 'unknown')}`
- Dynamic key failures: `{status.get('dynamic_key_failures', 'unknown')}`
- Screenshot regression failures: `{status.get('screenshot_regression_failures', 'unknown')}`
- UTF-8 files checked: `{status.get('utf8_files_checked', 'unknown')}`
- UTF-8 failures: `{status.get('utf8_failures', 'unknown')}`
...
def default_archive_path() -> Path:
    ...
    return ROOT.parent / f"lottery-probability-model_STEP150_2_PLAIN-BULGARIAN-USER-LANGUAGE-COMPLETE-TECHNICAL-SEPARATION_FULL-CLEAN_{stamp}.zip"


def main() -> int:
    parser = argparse.ArgumentParser(description="Regenerate, verify or package Step 150.2 release metadata")
