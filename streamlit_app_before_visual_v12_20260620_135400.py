...
def backup_historical_draws(action: str = "manual_update") -> Path | None:
    """Create a timestamped backup before any manual data change."""
    if not DATA_PATH.exists():
        return None

    safe_action = re.sub(r"[^a-zA-Z0-9_\-]+", "_", action).strip("_") or "manual_update"
    backup_dir = ROOT / "data" / "manual_backups"
    backup_dir.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_path = backup_dir / f"historical_draws_before_{safe_action}_{timestamp}.csv"

    suffix = 2
    while backup_path.exists():
        backup_path = backup_dir / f"historical_draws_before_{safe_action}_{timestamp}_{suffix}.csv"
        suffix += 1
...
def load_historical_csv_rows() -> tuple[list[dict[str, str]], list[str]]:
    """Load the historical CSV preserving its current columns."""
...
def write_historical_csv_rows(rows: list[dict[str, Any]], fieldnames: list[str]) -> None:
    """Write the historical CSV safely with preserved columns."""
...
def find_draw_rows(year: int, draw_number: int, draw_position: int) -> list[dict[str, str]]:
    """Find rows matching a concrete draw identity."""
    rows, _fieldnames = load_historical_csv_rows()
    matches: list[dict[str, str]] = []
    for row in rows:
        try:
            current_key = (int(row.get("year", 0)), int(row.get("draw_number", 0)), int(row.get("draw_position", 0)))
        except (TypeError, ValueError):
            continue
        if current_key == (int(year), int(draw_number), int(draw_position)):
            matches.append(row)
    return matches


def delete_manual_draw(year: int, draw_number: int, draw_position: int) -> tuple[bool, str]:
    """Delete exactly one draw by year + draw_number + draw_position."""
    rows, fieldnames = load_historical_csv_rows()
    if not rows:
        return False, "No historical rows were found."

    target_key = (int(year), int(draw_number), int(draw_position))
    kept_rows: list[dict[str, str]] = []
    deleted_rows: list[dict[str, str]] = []

    for row in rows:
        try:
            row_key = (int(row.get("year", 0)), int(row.get("draw_number", 0)), int(row.get("draw_position", 0)))
        except (TypeError, ValueError):
    ...
    if not deleted_rows:
        return False, f"No draw found for year={year}, draw_number={draw_number}, draw_position={draw_position}."

    if len(deleted_rows) > 1:
        return False, f"Safety stop: {len(deleted_rows)} rows match this draw key. Clean duplicates before deleting."

    backup_path = backup_historical_draws("manual_delete")
    write_historical_csv_rows(kept_rows, fieldnames)
    load_draws.clear()
    load_models.clear()

    numbers = [deleted_rows[0].get(f"n{index}", "") for index in range(1, 7)]
    return True, f"Deleted draw {year}/{draw_number}/{draw_position}: {' '.join(numbers)}. Backup: {backup_path}"


def find_latest_manual_backup() -> Path | None:
    """Find the latest backup that can safely be used for undo."""
    backup_dir = ROOT / "data" / "manual_backups"
    ...
    candidates = [
        path
        for path in backup_dir.glob("historical_draws_before_manual_*.csv")
        if "before_undo" not in path.name
    ]
    if not candidates:
        return None
...
def undo_last_manual_change() -> tuple[bool, str]:
    """Restore the historical CSV from the latest manual backup."""
    latest_backup = find_latest_manual_backup()
    if latest_backup is None:
        return False, "No manual backup was found for undo."

    current_backup = backup_historical_draws("undo_restore_current_state")
    shutil.copy2(latest_backup, DATA_PATH)
    load_draws.clear()
    load_models.clear()
    return True, f"Restored dataset from backup: {latest_backup}. Current state backup: {current_backup}"
...
def append_manual_draw(
    *,
    ...
    if DATA_PATH.exists():
        backup_path = backup_historical_draws("manual_update")
    else:
        backup_path = None
...
def retrain_all_models() -> tuple[bool, list[dict[str, Any]], Path]:
    """Run all training scripts after a successful manual draw update."""
            ...
            results.append(
                {
                    ...
                    "returncode": "missing",
                    "stdout": "",
                    "stderr": f"Missing training script: {script_name}",
                }
            )
            break
    ...
    log_path = REPORTS_DIR / "manual_update_retrain_log.md"
    with log_path.open("w", encoding="utf-8") as file:
        file.write("# Manual Update Retrain Log\n\n")
        file.write(f"Overall status: {'SUCCESS' if success else 'FAILED'}\n\n")
        for result in results:
            file.write(f"## {result['script']}\n\n")
    ...
    load_draws.clear()
    load_models.clear()
    return success, results, log_path


def render_training_results(results: list[dict[str, Any]]) -> None:
    """Display concise training results in the Streamlit UI."""
    for result in results:
        script = result["script"]
        returncode = result["returncode"]
        if returncode == 0:
            st.success(f"{script}: OK")
        else:
            st.error(f"{script}: FAILED ({returncode})")

        with st.expander(f"{script} log", expanded=False):
            if result.get("stdout"):
                st.text(result["stdout"][-5000:])
            if result.get("stderr"):
                st.text(result["stderr"][-5000:])



def decode_uploaded_file(uploaded_file: Any) -> str:
    """Decode an uploaded draw file into text."""
...
def parse_uploaded_draw_text(text: str) -> tuple[list[int] | None, str]:
    """
    Try to extract exactly one 6/49 winning combination from uploaded text.

    The parser intentionally prefers simple lines with exactly six valid numbers.
    If official text contains a prefix such as "Теглене 1:", the parser reads
    the numbers after the last colon to avoid treating draw numbers as balls.
    """
    cleaned = text.replace("\ufeff", " ").replace("\xa0", " ")
    lines = [line.strip() for line in cleaned.splitlines() if line.strip()]

    def valid_numbers_from(segment: str) -> list[int]:
        values = [int(token) for token in re.findall(r"(?<!\d)\d{1,2}(?!\d)", segment)]
        return [value for value in values if 1 <= value <= 49]

    candidates: list[tuple[int, list[int], str]] = []
    ...
    for line in lines:
        ...
        if "-" in line:
            segments.append(line.rsplit("-", 1)[-1])

        keyword_bonus = 0
        if any(word in lower for word in ["теглен", "печел", "числ", "winning", "draw", "combination", "комбинац"]):
            keyword_bonus = 10

        for segment in segments:
            values = valid_numbers_from(segment)
            unique_values = sorted(set(values))
            if len(unique_values) == 6:
                candidates.append((100 + keyword_bonus, unique_values, line))
            elif len(values) >= 6:
                first_six = values[:6]
                if len(set(first_six)) == 6:
                    candidates.append((60 + keyword_bonus, sorted(first_six), line))

    if candidates:
        candidates.sort(key=lambda item: item[0], reverse=True)
        return candidates[0][1], candidates[0][2]

    all_values = valid_numbers_from(cleaned)
    unique_all = sorted(set(all_values))
    if len(unique_all) == 6:
        return unique_all, "all uploaded text"

    return None, "Could not confidently detect exactly six unique numbers between 1 and 49."


def apply_uploaded_draw_to_form(uploaded_file: Any) -> tuple[bool, str]:
    """Parse an uploaded file and prefill the manual draw form."""
    try:
        text = decode_uploaded_file(uploaded_file)
    except Exception as exc:  # pragma: no cover - UI safety guard
        return False, f"Could not read uploaded file: {exc}"

    numbers, source = parse_uploaded_draw_text(text)
    if not numbers:
        return False, source

    for index, number in enumerate(numbers, start=1):
        st.session_state[f"manual_n{index}"] = int(number)

    st.session_state["manual_source_url"] = uploaded_file.name
    return True, f"Detected numbers {numbers} from: {source}"
...
def show_update_draws(rows: list[dict[str, Any]]) -> None:
    ...
    st.markdown(
        t(
            "Add a new official winning draw manually after the latest Bulgarian Toto 2 – 6/49 results are published. You can type the numbers or upload a downloaded result file from your computer.",
            "Добави нов официален печеливш тираж след публикуване на резултатите от Българско тото 2 – 6/49. Можеш да въведеш числата ръчно или да качиш свален файл с резултата от компютъра."
        )
    )
    ...
    st.markdown("### " + t("Upload draw from file", "Качи тираж от файл"))
    st.caption(
        t(
            "Upload a small .txt, .csv, or .json file containing the six winning numbers. The browser cannot read files from your PC automatically for security reasons, so you select the downloaded file here.",
            "Качи малък .txt, .csv или .json файл, който съдържа 6-те печеливши числа. Браузърът не може сам да чете файлове от компютъра ти от съображения за сигурност, затова избираш сваления файл тук."
        )
    )

    uploaded_draw_file = st.file_uploader(
        t("Choose downloaded result file", "Избери свален файл с резултат"),
        type=["txt", "csv", "json"],
        key="uploaded_draw_file",
    )

    upload_col1, upload_col2 = st.columns([1, 2])
    with upload_col1:
        upload_clicked = st.button(t("Upload draw", "Качи тираж"), use_container_width=True)
    with upload_col2:
        if uploaded_draw_file is not None:
            st.caption(t("Selected file:", "Избран файл:") + f" `{uploaded_draw_file.name}`")

    if upload_clicked:
        if uploaded_draw_file is None:
            st.warning(t("Choose a file first.", "Първо избери файл."))
        else:
            ok, message = apply_uploaded_draw_to_form(uploaded_draw_file)
            if ok:
                st.success(message if not is_bg() else "Числата са разпознати и попълнени във формата по-долу. Провери година/тираж/позиция и натисни ‘Запази новия тираж’." )
            else:
                st.error(message if not is_bg() else "Не успях да разпозная точно 6 валидни числа. Качи по-чист файл или въведи числата ръчно.")

    st.markdown("### " + t("Add new winning numbers", "Добави нови печеливши числа"))

    if "manual_draw_date" not in st.session_state:
        st.session_state["manual_draw_date"] = date.today()
    if "manual_year" not in st.session_state:
        st.session_state["manual_year"] = date.today().year
    if "manual_draw_number" not in st.session_state:
        st.session_state["manual_draw_number"] = 1
    if "manual_draw_position" not in st.session_state:
        st.session_state["manual_draw_position"] = 1
    if "manual_source_url" not in st.session_state:
        st.session_state["manual_source_url"] = "manual_entry"
    for index, default in enumerate([1, 2, 3, 4, 5, 6], start=1):
        st.session_state.setdefault(f"manual_n{index}", default)

    with st.form("manual_draw_form"):
        col_a, col_b, col_c, col_d = st.columns(4)
        with col_a:
            draw_date = st.date_input(t("Draw date", "Дата на теглене"), key="manual_draw_date")
        with col_b:
            year = st.number_input(t("Year", "Година"), min_value=1958, max_value=2100, step=1, key="manual_year")
        with col_c:
            draw_number = st.number_input(t("Draw number", "Номер на тираж"), min_value=0, max_value=999, step=1, key="manual_draw_number")
        with col_d:
            draw_position = st.number_input(t("Draw position", "Позиция на теглене"), min_value=1, max_value=10, step=1, key="manual_draw_position")
        ...
        for index, column in enumerate(number_cols, start=1):
            with column:
                numbers.append(st.number_input(f"N{index}", min_value=1, max_value=49, step=1, key=f"manual_n{index}"))

        source_url = st.text_input(t("Source note / URL", "Източник / бележка / URL"), key="manual_source_url")
        auto_retrain = st.checkbox(
            t("Automatically update models after saving", "Автоматично обнови моделите след запис"),
            value=True,
            help=t(
                "Runs all training scripts immediately after the new draw is saved. This may take a little time, but the dashboard will update without manual terminal commands.",
                "Пуска всички training скриптове веднага след запис на новия тираж. Може да отнеме малко време, но таблото ще се обнови без ръчни команди в терминала."
            ),
        )
    ...
    if submitted:
        ...
        if ok:
            ...
            if auto_retrain:
                with st.spinner(t("Updating models...", "Обновявам моделите...")):
                    retrain_ok, retrain_results, log_path = retrain_all_models()

                render_training_results(retrain_results)

                if retrain_ok:
                    st.success(
                        t(
                            "Models updated successfully. Refresh the dashboard to see the new recommendations.",
                            "Моделите са обновени успешно. Обнови таблото, за да видиш новите препоръки."
                        )
                    )
                    st.info(t("Retrain log saved to:", "Логът от обновяването е записан в:") + f" `{log_path}`")
                    st.button(t("Refresh app now", "Обнови приложението сега"), on_click=st.rerun)
                else:
                    st.error(
                        t(
                            "The draw was saved, but one training script failed. Check the log below and the saved retrain log.",
                            "Тиражът е запазен, но един training скрипт даде грешка. Провери логовете по-долу и записания retrain log."
                        )
                    )
                    st.info(t("Retrain log saved to:", "Логът от обновяването е записан в:") + f" `{log_path}`")
            else:
                st.info(
                    t(
                        "Automatic model update is disabled. You can retrain manually from the terminal:",
                        "Автоматичното обновяване е изключено. Можеш да преобучиш моделите ръчно от терминала:"
                    )
    ...
    st.markdown("### " + t("Correction / delete draw", "Корекция / изтриване на тираж"))
    st.warning(
        t(
            "Delete only by year + draw number + draw position, not by the six numbers. The same number combination can theoretically appear again in another draw.",
            "Трий само по година + номер на тираж + позиция, не по самите 6 числа. Една и съща комбинация теоретично може да се падне отново в друг тираж."
        )
    )

    undo_col1, undo_col2 = st.columns([1, 2])
    with undo_col1:
        undo_clicked = st.button(
            t("Undo last manual change", "Върни последната ръчна промяна"),
            use_container_width=True,
        )
    with undo_col2:
        latest_backup = find_latest_manual_backup()
        if latest_backup is not None:
            st.caption(t("Latest available backup:", "Последен наличен backup:") + f" `{latest_backup.name}`")
        else:
            st.caption(t("No manual backup found yet.", "Все още няма намерен ръчен backup."))

    if undo_clicked:
        undo_ok, undo_message = undo_last_manual_change()
        if undo_ok:
            st.success(
                undo_message
                if not is_bg()
                else "Последната ръчна промяна е върната от backup. Сега обнови моделите, за да съвпадат с данните."
            )
            with st.spinner(t("Updating models...", "Обновявам моделите...")):
                retrain_ok, retrain_results, log_path = retrain_all_models()
            render_training_results(retrain_results)
            if retrain_ok:
                st.success(t("Models updated after undo.", "Моделите са обновени след връщането."))
                st.info(t("Retrain log saved to:", "Логът от обновяването е записан в:") + f" `{log_path}`")
                st.button(t("Refresh app now", "Обнови приложението сега"), on_click=st.rerun, key="refresh_after_undo")
            else:
                st.error(t("Undo succeeded, but retraining failed. Check the log.", "Връщането мина успешно, но обновяването на моделите даде грешка. Провери лога."))
                st.info(t("Retrain log saved to:", "Логът от обновяването е записан в:") + f" `{log_path}`")
        else:
            st.error(undo_message if not is_bg() else "Няма намерен backup за връщане.")

    with st.form("delete_draw_form"):
        del_col1, del_col2, del_col3 = st.columns(3)
        with del_col1:
            delete_year = st.number_input(t("Year to delete", "Година за изтриване"), min_value=1958, max_value=2100, value=date.today().year, step=1)
        with del_col2:
            delete_draw_number = st.number_input(t("Draw number to delete", "Номер на тираж за изтриване"), min_value=0, max_value=999, value=1, step=1)
        with del_col3:
            delete_draw_position = st.number_input(t("Draw position to delete", "Позиция за изтриване"), min_value=1, max_value=10, value=1, step=1)

        auto_retrain_delete = st.checkbox(
            t("Automatically update models after delete", "Автоматично обнови моделите след изтриване"),
            value=True,
            key="auto_retrain_delete",
        )
        confirm_delete = st.checkbox(
            t("I understand this will delete exactly this draw key", "Разбирам, че това ще изтрие точно този ключ на тираж"),
            value=False,
            key="confirm_delete_draw",
        )
        delete_submitted = st.form_submit_button(t("Delete this draw", "Изтрий този тираж"))

    if delete_submitted:
        if not confirm_delete:
            st.error(t("Confirm the delete checkbox first.", "Първо потвърди отметката за изтриване."))
        else:
            preview_rows = find_draw_rows(int(delete_year), int(delete_draw_number), int(delete_draw_position))
            if preview_rows:
                preview_numbers = " ".join(str(preview_rows[0].get(f"n{index}", "")) for index in range(1, 7))
                st.info(t("Matched draw:", "Намерен тираж:") + f" `{int(delete_year)}/{int(delete_draw_number)}/{int(delete_draw_position)} — {preview_numbers}`")

            delete_ok, delete_message = delete_manual_draw(
                int(delete_year),
                int(delete_draw_number),
                int(delete_draw_position),
            )
            if delete_ok:
                st.success(delete_message if not is_bg() else "Тиражът е изтрит успешно. Направен е backup преди изтриване.")
                if auto_retrain_delete:
                    with st.spinner(t("Updating models...", "Обновявам моделите...")):
                        retrain_ok, retrain_results, log_path = retrain_all_models()
                    render_training_results(retrain_results)
                    if retrain_ok:
                        st.success(t("Models updated after delete.", "Моделите са обновени след изтриването."))
                        st.info(t("Retrain log saved to:", "Логът от обновяването е записан в:") + f" `{log_path}`")
                        st.button(t("Refresh app now", "Обнови приложението сега"), on_click=st.rerun, key="refresh_after_delete")
                    else:
                        st.error(t("The draw was deleted, but retraining failed. Check the log.", "Тиражът е изтрит, но обновяването на моделите даде грешка. Провери лога."))
                        st.info(t("Retrain log saved to:", "Логът от обновяването е записан в:") + f" `{log_path}`")
            else:
                st.error(delete_message if not is_bg() else "Не е намерен точно един тираж с тези данни или има конфликт.")
...
def _run_project_script(script_name: str) -> tuple[bool, str]:
    """Run a project script and return success plus combined output."""
    try:
        ...
        return False, f"Could not run {script_name}: {exc}"
...
def show_advanced_lab(rows: list[dict[str, Any]], models: dict[str, Any]) -> None:
    st.header(t("Advanced Statistical Lab", "Разширена статистическа лаборатория"))
    st.caption(
        t(
            "Adds time-decay, Bayesian smoothing, fairness tests, pair/triple significance, portfolio optimization and rolling backtesting.",
            "Добавя time-decay, Bayesian smoothing, fairness тестове, значимост на двойки/тройки, портфолио оптимизация и rolling backtesting.",
        )
    )

    st.info(
        t(
            "Important: these methods rank combinations statistically. They do not change the true jackpot odds for an exact 6-number ticket.",
            "Важно: тези методи ранжират комбинации статистически. Те не променят реалния шанс за точен фиш с 6 числа.",
        )
    )

    action_left, action_right = st.columns(2)
    with action_left:
        if st.button(t("Train advanced ensemble", "Обучи разширения ансамбъл"), use_container_width=True):
            with st.spinner(t("Training advanced model...", "Обучавам разширения модел...")):
                ok, output = _run_project_script("train_advanced_model.py")
            if ok:
                st.success(t("Advanced model trained successfully.", "Разширеният модел е обучен успешно."))
            else:
                st.error(t("Advanced training failed.", "Обучението даде грешка."))
            st.code(output or "No output", language="text")
            st.button(t("Refresh app now", "Обнови приложението сега"), on_click=st.rerun, key="refresh_advanced_train")

    with action_right:
        if st.button(t("Run backtesting engine", "Пусни Backtesting engine"), use_container_width=True):
            with st.spinner(t("Running rolling backtest...", "Изпълнявам rolling backtest...")):
                ok, output = _run_project_script("run_advanced_backtest.py")
            if ok:
                st.success(t("Backtest completed successfully.", "Backtest-ът завърши успешно."))
            else:
                st.error(t("Backtest failed.", "Backtest-ът даде грешка."))
            st.code(output or "No output", language="text")
            st.button(t("Refresh app now", "Обнови приложението сега"), on_click=st.rerun, key="refresh_advanced_backtest")

    advanced_model = _load_json_file(MODELS_DIR / "lottery_advanced_ensemble_model.json")
    backtest = _load_json_file(MODELS_DIR / "lottery_advanced_backtest.json")

    if not advanced_model:
        st.warning(
            t(
                "Advanced model is not trained yet. Click Train advanced ensemble first.",
                "Разширеният модел още не е обучен. Първо натисни Обучи разширения ансамбъл.",
            )
        )
        st.markdown("### " + t("What will be added?", "Какво добавяме?"))
        st.markdown(
            t(
                """
- **Time-decay model** — gives more weight to recent draws.
- **Bayesian smoothing** — avoids overreacting to random noise.
- **Chi-square fairness test** — checks whether frequencies look fair.
- **Pair/triple significance** — checks co-occurrences against random expectation.
- **Human-pattern avoidance** — penalizes common human choices like only dates or obvious sequences.
- **Portfolio optimizer** — gives diversified tickets, not ten nearly identical ones.
- **Backtesting engine** — tests strategies on past future draws using only earlier data.
""",
                """
- **Time-decay модел** — дава по-голяма тежест на последните тиражи.
- **Bayesian smoothing** — пази модела от прекалена реакция към случаен шум.
- **Chi-square fairness тест** — проверява дали честотите изглеждат нормално случайни.
- **Значимост на двойки/тройки** — сравнява съвместните появявания с очакваното при случайност.
- **Избягване на човешки шаблони** — наказва дати, очевидни последователности и прекалено “красиви” фишове.
- **Портфолио оптимизатор** — дава разнообразни фишове, не десет почти еднакви.
- **Backtesting engine** — връща се назад и тества стратегията само с данни, налични преди съответния тираж.
""",
            )
        )
        return

    fairness = advanced_model.get("fairness_test", {})
    metric_cols = st.columns(4)
    metric_cols[0].metric(t("Training draws", "Тегления за обучение"), advanced_model.get("training_draws", "—"))
    metric_cols[1].metric(t("Candidates", "Кандидат комбинации"), advanced_model.get("candidate_count", "—"))
    metric_cols[2].metric(t("Fairness p-value", "Fairness p-value"), f"{float(fairness.get('p_value', 0.0)):.4f}")
    metric_cols[3].metric(t("True jackpot odds", "Реален шанс за джакпот"), advanced_model.get("theoretical_jackpot_odds", "1 in 13983816"))

    st.caption(fairness.get("interpretation", ""))

    tab_recs, tab_portfolio, tab_numbers, tab_pairs, tab_backtest = st.tabs(
        [
            t("Advanced recommendations", "Разширени препоръки"),
            t("Portfolio", "Портфолио"),
            t("Number signals", "Сигнали по числа"),
            t("Pair / triple significance", "Двойки / тройки"),
            t("Backtesting", "Backtesting"),
        ]
    )

    with tab_recs:
        recommendations = advanced_model.get("recommended_combinations", [])
        if recommendations:
            df = pd.DataFrame(
                [
                    {
                        "rank": item.get("relative_rank"),
                        "numbers": " ".join(str(number) for number in item.get("numbers", [])),
                        "confidence": f"{float(item.get('confidence_score', 0.0)):.2f}/100",
                        "relative_probability": _format_percent(item.get("relative_model_probability", 0.0), 4),
                        "top_percent": f"{float(item.get('relative_top_percent', 0.0)):.2f}%",
                        "time_decay": f"{float(item.get('sub_scores', {}).get('time_decay_score', 0.0)):.3f}",
                        "bayesian": f"{float(item.get('sub_scores', {}).get('bayesian_score', 0.0)):.3f}",
                        "gap": f"{float(item.get('sub_scores', {}).get('gap_score', 0.0)):.3f}",
                        "human_avoidance": f"{float(item.get('sub_scores', {}).get('human_pattern_avoidance_score', 0.0)):.3f}",
                    }
                    for item in recommendations[:20]
                ]
            )
            st.dataframe(localize_dataframe_columns(df), hide_index=True, use_container_width=True)
        else:
            st.info(t("No advanced recommendations found.", "Няма намерени разширени препоръки."))

    with tab_portfolio:
        portfolio = advanced_model.get("portfolio_recommendations", [])
        st.caption(
            t(
                "Portfolio mode tries to avoid giving many nearly identical tickets.",
                "Портфолио режимът се опитва да избегне много почти еднакви фишове.",
            )
        )
        if portfolio:
                ...
                [
                    {
                        ...
                        "numbers": " ".join(str(number) for number in item.get("numbers", [])),
                        "confidence": f"{float(item.get('confidence_score', 0.0)):.2f}/100",
                        "reason": item.get("portfolio_reason", ""),
                    }
                    for index, item in enumerate(portfolio, start=1)
                ]
            )
            st.dataframe(localize_dataframe_columns(portfolio_df), hide_index=True, use_container_width=True)
        else:
            st.info(t("No portfolio recommendations found.", "Няма намерени портфолио препоръки."))

    with tab_numbers:
        number_scores = advanced_model.get("top_number_scores", [])
        if number_scores:
            number_df = pd.DataFrame(
                [
                    {
                        "number": item.get("number"),
                        "advanced_score": round(float(item.get("advanced_number_score", 0.0)), 4),
                        "time_decay": round(float(item.get("time_decay_score", 0.0)), 4),
                        "bayesian": round(float(item.get("bayesian_score", 0.0)), 4),
                        "gap": round(float(item.get("gap_score", 0.0)), 4),
                        "z_score": round(float(item.get("z_score", 0.0)), 2),
                        "current_gap": item.get("current_gap"),
                    }
                    for item in number_scores
                ]
            )
            st.dataframe(localize_dataframe_columns(number_df), hide_index=True, use_container_width=True)
        else:
            st.info(t("No number-level signals found.", "Няма намерени сигнали по числа."))

    with tab_pairs:
        left, right = st.columns(2)
        with left:
            st.markdown("#### " + t("Top pairs", "Топ двойки"))
            pairs = advanced_model.get("top_pair_significance", [])
            if pairs:
                pair_df = pd.DataFrame(
                    [
                        {
                            "pair": "-".join(str(number) for number in item.get("pair", [])),
                            "count": item.get("count"),
                            "expected": round(float(item.get("expected", 0.0)), 2),
                            "z_score": round(float(item.get("z_score", 0.0)), 2),
                        }
                        for item in pairs
                    ]
                )
                st.dataframe(localize_dataframe_columns(pair_df), hide_index=True, use_container_width=True)
        with right:
            st.markdown("#### " + t("Top triples", "Топ тройки"))
            triples = advanced_model.get("top_triple_significance", [])
            if triples:
                triple_df = pd.DataFrame(
                    [
                        {
                            "triple": "-".join(str(number) for number in item.get("triple", [])),
                            "count": item.get("count"),
                            "expected": round(float(item.get("expected", 0.0)), 2),
                            "z_score": round(float(item.get("z_score", 0.0)), 2),
                        }
                ...
                st.dataframe(localize_dataframe_columns(triple_df), hide_index=True, use_container_width=True)

    with tab_backtest:
        if not backtest:
            st.warning(
                t(
                    "Backtest not run yet. Click Run backtesting engine above.",
                    "Backtest още не е пуснат. Натисни Пусни Backtesting engine горе.",
                )
            )
        else:
            st.metric(t("Tested draws", "Тествани тиражи"), backtest.get("tested_draws", "—"))
            st.info(backtest.get("conclusion", ""))
            averages = backtest.get("averages", {})
            hit_rates = backtest.get("hit_rates", {})
            if averages:
                backtest_df = pd.DataFrame(
                    [
                        {
                            "strategy": name,
                            "average_matches": round(float(avg), 4),
                            "hit_rate_3_plus": _format_percent(hit_rates.get(name, {}).get("at_least_3", 0.0), 2),
                            "hit_rate_4_plus": _format_percent(hit_rates.get(name, {}).get("at_least_4", 0.0), 2),
                        }
                        for name, avg in averages.items()
                    ]
                )
                st.dataframe(localize_dataframe_columns(backtest_df), hide_index=True, use_container_width=True)

            details = backtest.get("details", [])[-20:]
            if details:
                    ...
                    [
                        {
                            ...
                            "date": item.get("date"),
                            "actual": " ".join(str(number) for number in item.get("actual_numbers", [])),
                            "advanced_ticket": " ".join(str(number) for number in item.get("tickets", {}).get("advanced", {}).get("numbers", [])),
                            "advanced_matches": item.get("tickets", {}).get("advanced", {}).get("matches"),
                            "random_matches": item.get("tickets", {}).get("random", {}).get("matches"),
                        }
                        for item in details
                    ]
                )
                st.dataframe(localize_dataframe_columns(detail_df), hide_index=True, use_container_width=True)
...
PAGES = {
    ...
    "advanced": ("Advanced Lab", "Разширена лаборатория"),
