...
# === LOTTERY BULGARIAN UI FIX V27 START ===
# Corrected Bulgarian UI localization layer.
# Fixes the v26 dataframe recursion bug.
try:
    import pandas as _bg27_pd
    import inspect as _bg27_inspect
except Exception:  # pragma: no cover
    _bg27_pd = None
    _bg27_inspect = None


def _bg27_is_bulgarian() -> bool:
    try:
...
def _bg27_text(value):
    if not _bg27_is_bulgarian() or not isinstance(value, str):
        return value
    if value in _BG27_EXACT:
        return _BG27_EXACT[value]
    out = value
    for src, dst in _BG27_REPLACE:
        out = out.replace(src, dst)
    return out


def _bg27_column(col):
    raw = str(col)
    key = raw.strip().lower()
    if key in _BG27_COLUMN_MAP:
        return _BG27_COLUMN_MAP[key]
    return _bg27_text(raw)


def _bg27_translate_dataframe(data):
    if not _bg27_is_bulgarian() or _bg27_pd is None:
        return data
    try:
        if not isinstance(data, _bg27_pd.DataFrame):
            return data
        df = data.copy()
        df = df.rename(columns={col: _bg27_column(col) for col in df.columns})
        for col in df.columns:
            if df[col].dtype == "object":
                df[col] = df[col].map(lambda x: _bg27_text(x) if isinstance(x, str) else x)
        return df
    except Exception:
        return data
...
if not getattr(st, "_lottery_bulgarian_ui_fix_v27", False):
    st._lottery_bulgarian_ui_fix_v27 = True

    _bg27_orig_markdown = st.markdown
    _bg27_orig_write = st.write
    _bg27_orig_caption = st.caption
    _bg27_orig_info = st.info
    _bg27_orig_success = st.success
    _bg27_orig_warning = st.warning
    _bg27_orig_error = st.error
    _bg27_orig_title = st.title
    _bg27_orig_header = st.header
    _bg27_orig_subheader = st.subheader
    _bg27_orig_metric = st.metric
    _bg27_orig_button = st.button
    _bg27_orig_checkbox = st.checkbox
    _bg27_orig_radio = st.radio
    _bg27_orig_selectbox = st.selectbox
    _bg27_orig_multiselect = st.multiselect
    _bg27_orig_text_input = st.text_input
    _bg27_orig_number_input = st.number_input
    _bg27_orig_file_uploader = st.file_uploader
    _bg27_orig_download_button = st.download_button
    _bg27_orig_dataframe = st.dataframe
    _bg27_orig_table = st.table

    def _bg27_markdown(body, *args, **kwargs):
        return _bg27_orig_markdown(_bg27_text(body), *args, **kwargs)

    def _bg27_write(*args, **kwargs):
        args = tuple(_bg27_text(a) if isinstance(a, str) else a for a in args)
        return _bg27_orig_write(*args, **kwargs)

    def _bg27_caption(body, *args, **kwargs):
        return _bg27_orig_caption(_bg27_text(body), *args, **kwargs)

    def _bg27_info(body, *args, **kwargs):
        return _bg27_orig_info(_bg27_text(body), *args, **kwargs)

    def _bg27_success(body, *args, **kwargs):
        return _bg27_orig_success(_bg27_text(body), *args, **kwargs)

    def _bg27_warning(body, *args, **kwargs):
        return _bg27_orig_warning(_bg27_text(body), *args, **kwargs)

    def _bg27_error(body, *args, **kwargs):
        return _bg27_orig_error(_bg27_text(body), *args, **kwargs)

    def _bg27_title(body, *args, **kwargs):
        return _bg27_orig_title(_bg27_text(body), *args, **kwargs)

    def _bg27_header(body, *args, **kwargs):
        return _bg27_orig_header(_bg27_text(body), *args, **kwargs)

    def _bg27_subheader(body, *args, **kwargs):
        return _bg27_orig_subheader(_bg27_text(body), *args, **kwargs)

    def _bg27_metric(label, value, delta=None, *args, **kwargs):
        return _bg27_orig_metric(_bg27_text(label), _bg27_text(value), _bg27_text(delta), *args, **kwargs)

    def _bg27_button(label, *args, **kwargs):
        return _bg27_orig_button(_bg27_text(label), *args, **kwargs)

    def _bg27_checkbox(label, *args, **kwargs):
        return _bg27_orig_checkbox(_bg27_text(label), *args, **kwargs)

    def _bg27_radio(label, options, *args, **kwargs):
        return _bg27_orig_radio(_bg27_text(label), options, *args, **kwargs)

    def _bg27_selectbox(label, options, *args, **kwargs):
        return _bg27_orig_selectbox(_bg27_text(label), options, *args, **kwargs)

    def _bg27_multiselect(label, options, *args, **kwargs):
        _bg27_css()
        if _bg27_is_bulgarian() and _bg27_inspect is not None:
            try:
                if "placeholder" in _bg27_inspect.signature(_bg27_orig_multiselect).parameters:
                    kwargs.setdefault("placeholder", "Избери числа")
            except Exception:
                pass
        return _bg27_orig_multiselect(_bg27_text(label), options, *args, **kwargs)

    def _bg27_text_input(label, *args, **kwargs):
        if "value" in kwargs and isinstance(kwargs.get("value"), str):
            kwargs["value"] = _bg27_text(kwargs["value"])
        if "placeholder" in kwargs and isinstance(kwargs.get("placeholder"), str):
            kwargs["placeholder"] = _bg27_text(kwargs["placeholder"])
        return _bg27_orig_text_input(_bg27_text(label), *args, **kwargs)

    def _bg27_number_input(label, *args, **kwargs):
        return _bg27_orig_number_input(_bg27_text(label), *args, **kwargs)

    def _bg27_file_uploader(label, *args, **kwargs):
        _bg27_css()
        return _bg27_orig_file_uploader(_bg27_text(label), *args, **kwargs)

    def _bg27_download_button(label, *args, **kwargs):
        return _bg27_orig_download_button(_bg27_text(label), *args, **kwargs)

    def _bg27_dataframe_widget(data=None, *args, **kwargs):
        return _bg27_orig_dataframe(_bg27_translate_dataframe(data), *args, **kwargs)

    def _bg27_table_widget(data=None, *args, **kwargs):
        return _bg27_orig_table(_bg27_translate_dataframe(data), *args, **kwargs)

    st.markdown = _bg27_markdown
    st.write = _bg27_write
    st.caption = _bg27_caption
    st.info = _bg27_info
    st.success = _bg27_success
    st.warning = _bg27_warning
    st.error = _bg27_error
    st.title = _bg27_title
    st.header = _bg27_header
    st.subheader = _bg27_subheader
    st.metric = _bg27_metric
    st.button = _bg27_button
    st.checkbox = _bg27_checkbox
    st.radio = _bg27_radio
    st.selectbox = _bg27_selectbox
    st.multiselect = _bg27_multiselect
    st.text_input = _bg27_text_input
    st.number_input = _bg27_number_input
    st.file_uploader = _bg27_file_uploader
    st.download_button = _bg27_download_button
    st.dataframe = _bg27_dataframe_widget
    st.table = _bg27_table_widget
# === LOTTERY BULGARIAN UI FIX V27 END ===
