...
# === LOTTERY TABLE COLUMNS BG V28 START ===
# Focused Bulgarian column/header polish.
# This layer changes displayed text only and avoids the dataframe recursion bug.
try:
    import pandas as _bg28_pd
    import inspect as _bg28_inspect
except Exception:  # pragma: no cover
    _bg28_pd = None
    _bg28_inspect = None


def _bg28_is_bulgarian() -> bool:
    try:
...
_BG28_TEXT_MAP = {
    ...
    # Table columns - historical stats
    "number": "Число",
    "Number": "Число",
    "count": "Брой",
    "Count": "Брой",
    ...
    # Visible headings and card text
...
def _bg28_text(value):
    if not _bg28_is_bulgarian() or not isinstance(value, str):
        return value
    if value in _BG28_TEXT_MAP:
        return _BG28_TEXT_MAP[value]
    out = value
    for src, dst in _BG28_REPLACE:
        out = out.replace(src, dst)
    return out


def _bg28_column(col):
    raw = str(col)
    if raw in _BG28_TEXT_MAP:
        return _BG28_TEXT_MAP[raw]
    key = raw.strip().lower()
    if key in _BG28_TEXT_MAP:
        return _BG28_TEXT_MAP[key]
    return _bg28_text(raw)


def _bg28_translate_dataframe(data):
    if not _bg28_is_bulgarian() or _bg28_pd is None:
        return data
    try:
        if not isinstance(data, _bg28_pd.DataFrame):
            return data
        df = data.copy()
        df = df.rename(columns={col: _bg28_column(col) for col in df.columns})
        for col in df.columns:
            if df[col].dtype == "object":
                df[col] = df[col].map(lambda x: _bg28_text(x) if isinstance(x, str) else x)
        return df
    except Exception:
        return data
...
if not getattr(st, "_lottery_table_columns_bg_v28", False):
    st._lottery_table_columns_bg_v28 = True

    _bg28_orig_markdown = st.markdown
    _bg28_orig_write = st.write
    _bg28_orig_caption = st.caption
    _bg28_orig_info = st.info
    _bg28_orig_success = st.success
    _bg28_orig_warning = st.warning
    _bg28_orig_error = st.error
    _bg28_orig_title = st.title
    _bg28_orig_header = st.header
    _bg28_orig_subheader = st.subheader
    _bg28_orig_metric = st.metric
    _bg28_orig_button = st.button
    _bg28_orig_checkbox = st.checkbox
    _bg28_orig_radio = st.radio
    _bg28_orig_selectbox = st.selectbox
    _bg28_orig_multiselect = st.multiselect
    _bg28_orig_text_input = st.text_input
    _bg28_orig_number_input = st.number_input
    _bg28_orig_file_uploader = st.file_uploader
    _bg28_orig_download_button = st.download_button
    _bg28_orig_dataframe = st.dataframe
    _bg28_orig_table = st.table

    def _bg28_markdown(body, *args, **kwargs):
        return _bg28_orig_markdown(_bg28_text(body), *args, **kwargs)

    def _bg28_write(*args, **kwargs):
        args = tuple(_bg28_text(a) if isinstance(a, str) else a for a in args)
        return _bg28_orig_write(*args, **kwargs)

    def _bg28_caption(body, *args, **kwargs):
        return _bg28_orig_caption(_bg28_text(body), *args, **kwargs)

    def _bg28_info(body, *args, **kwargs):
        return _bg28_orig_info(_bg28_text(body), *args, **kwargs)

    def _bg28_success(body, *args, **kwargs):
        return _bg28_orig_success(_bg28_text(body), *args, **kwargs)

    def _bg28_warning(body, *args, **kwargs):
        return _bg28_orig_warning(_bg28_text(body), *args, **kwargs)

    def _bg28_error(body, *args, **kwargs):
        return _bg28_orig_error(_bg28_text(body), *args, **kwargs)

    def _bg28_title(body, *args, **kwargs):
        return _bg28_orig_title(_bg28_text(body), *args, **kwargs)

    def _bg28_header(body, *args, **kwargs):
        return _bg28_orig_header(_bg28_text(body), *args, **kwargs)

    def _bg28_subheader(body, *args, **kwargs):
        return _bg28_orig_subheader(_bg28_text(body), *args, **kwargs)

    def _bg28_metric(label, value, delta=None, *args, **kwargs):
        return _bg28_orig_metric(_bg28_text(label), _bg28_text(value), _bg28_text(delta), *args, **kwargs)

    def _bg28_button(label, *args, **kwargs):
        return _bg28_orig_button(_bg28_text(label), *args, **kwargs)

    def _bg28_checkbox(label, *args, **kwargs):
        return _bg28_orig_checkbox(_bg28_text(label), *args, **kwargs)

    def _bg28_radio(label, options, *args, **kwargs):
        return _bg28_orig_radio(_bg28_text(label), options, *args, **kwargs)

    def _bg28_selectbox(label, options, *args, **kwargs):
        return _bg28_orig_selectbox(_bg28_text(label), options, *args, **kwargs)

    def _bg28_multiselect(label, options, *args, **kwargs):
        _bg28_css()
        if _bg28_is_bulgarian() and _bg28_inspect is not None:
            try:
                if "placeholder" in _bg28_inspect.signature(_bg28_orig_multiselect).parameters:
                    kwargs.setdefault("placeholder", "Избери числа")
            except Exception:
                pass
        return _bg28_orig_multiselect(_bg28_text(label), options, *args, **kwargs)

    def _bg28_text_input(label, *args, **kwargs):
        if "value" in kwargs and isinstance(kwargs.get("value"), str):
            kwargs["value"] = _bg28_text(kwargs["value"])
        if "placeholder" in kwargs and isinstance(kwargs.get("placeholder"), str):
            kwargs["placeholder"] = _bg28_text(kwargs["placeholder"])
        return _bg28_orig_text_input(_bg28_text(label), *args, **kwargs)

    def _bg28_number_input(label, *args, **kwargs):
        return _bg28_orig_number_input(_bg28_text(label), *args, **kwargs)

    def _bg28_file_uploader(label, *args, **kwargs):
        _bg28_css()
        return _bg28_orig_file_uploader(_bg28_text(label), *args, **kwargs)

    def _bg28_download_button(label, *args, **kwargs):
        return _bg28_orig_download_button(_bg28_text(label), *args, **kwargs)

    def _bg28_dataframe_widget(data=None, *args, **kwargs):
        return _bg28_orig_dataframe(_bg28_translate_dataframe(data), *args, **kwargs)

    def _bg28_table_widget(data=None, *args, **kwargs):
        return _bg28_orig_table(_bg28_translate_dataframe(data), *args, **kwargs)

    st.markdown = _bg28_markdown
    st.write = _bg28_write
    st.caption = _bg28_caption
    st.info = _bg28_info
    st.success = _bg28_success
    st.warning = _bg28_warning
    st.error = _bg28_error
    st.title = _bg28_title
    st.header = _bg28_header
    st.subheader = _bg28_subheader
    st.metric = _bg28_metric
    st.button = _bg28_button
    st.checkbox = _bg28_checkbox
    st.radio = _bg28_radio
    st.selectbox = _bg28_selectbox
    st.multiselect = _bg28_multiselect
    st.text_input = _bg28_text_input
    st.number_input = _bg28_number_input
    st.file_uploader = _bg28_file_uploader
    st.download_button = _bg28_download_button
    st.dataframe = _bg28_dataframe_widget
    st.table = _bg28_table_widget
# === LOTTERY TABLE COLUMNS BG V28 END ===
