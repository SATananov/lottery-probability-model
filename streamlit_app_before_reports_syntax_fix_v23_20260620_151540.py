...
# === LOTTERY REPORTS BG VISUAL V22 START ===
# Visual Bulgarian renderer for backtesting report text.
try:
    import re as _lr22_re
    import pandas as _lr22_pd
...
def _lr22_balls(numbers, size=34):
    nums = []
    for x in numbers or []:
        try:
            nums.append(int(x))
            ...
            nums.append(x)
    parts = [
        "<div style='display:flex;flex-wrap:wrap;gap:6px;align-items:center;margin:4px 0 8px;'>"
    ]
    for n in nums:
        parts.append(
...
def _lr22_parse_strategy_rows(text_value):
    if not _lr22_re:
        return []
    rows = []
    pattern = _lr22_re.compile(
        r"^\\s*([A-Za-z_]+):\\s*avg=([0-9.]+),\\s*>=3=([0-9.]+)%,\\s*>=4=([0-9.]+)%",
        _lr22_re.M,
    )
    for name, avg, ge3, ge4 in pattern.findall(text_value):
        rows.append({
            "Стратегия": _lr22_strategy_label(name),
            "Код": name,
...
def _lr22_parse_recent_draws(text_value):
    if not _lr22_re:
        return []
    rows = []
    pattern = _lr22_re.compile(
    ...
    for draw, date, actual, advanced, adv_matches, random_ticket, rnd_matches in pattern.findall(text_value):
        rows.append({
            "Тираж": int(draw),
            "Дата": date.strip() or "-",
            "Реални числа": _lr22_nums(actual),
            "Advanced фиш": _lr22_nums(advanced),
            "Advanced съвпадения": int(adv_matches),
            "Random фиш": _lr22_nums(random_ticket),
            "Random съвпадения": int(rnd_matches),
        })
    return rows


def _lr22_render_report(text_value):
    if not isinstance(text_value, str):
        return False

    if "Recent tested draws" not in text_value and "actual=[" not in text_value:
        return False

    st.markdown("## Отчет от backtesting")

    tested = None
    best_strategy = None

    if _lr22_re:
        m = _lr22_re.search(r"Tested draws:\\s*(\\d+)", text_value)
        if m:
            tested = int(m.group(1))
        m = _lr22_re.search(r"Best strategy:\\s*([A-Za-z_]+)", text_value)
        if m:
            best_strategy = m.group(1)

    cols = st.columns(3)
    cols[0].metric("Тествани тиражи", tested if tested is not None else "-")
    cols[1].metric("Най-добра стратегия", _lr22_strategy_label(best_strategy) if best_strategy else "-")
    cols[2].metric("Важно", "Не е доказателство", help="Backtest-ът е проверка назад във времето, не гаранция за бъдещи тегления.")

    strategy_rows = _lr22_parse_strategy_rows(text_value)
    if strategy_rows and _lr22_pd is not None:
        st.markdown("### Сравнение на стратегиите")
        df = _lr22_pd.DataFrame(strategy_rows)
        st.dataframe(df, width="stretch", hide_index=True)
        try:
            chart_df = df.set_index("Стратегия")[["Средни съвпадения"]]
    ...
    if best_strategy:
        st.info(
            ...
            f"{_lr22_strategy_label(best_strategy)}. "
            "Това е проверка на модела, не доказателство, че бъдещи тегления са предсказуеми."
        )

    recent_rows = _lr22_parse_recent_draws(text_value)
    if recent_rows:
        st.markdown("### Последни тествани тиражи")

        if _lr22_pd is not None:
            table_rows = []
...
if not getattr(st, "_lottery_reports_bg_visual_v22", False):
    st._lottery_reports_bg_visual_v22 = True

    _lr22_original_markdown = st.markdown
    _lr22_original_write = st.write
    _lr22_original_code = st.code

    def _lr22_markdown(body, *args, **kwargs):
        try:
            if isinstance(body, str) and _lr22_render_report(body):
                return
        except Exception:
            pass
        return _lr22_original_markdown(body, *args, **kwargs)

    def _lr22_write(*args, **kwargs):
        try:
            if len(args) == 1 and isinstance(args[0], str) and _lr22_render_report(args[0]):
                return
        except Exception:
            pass
        return _lr22_original_write(*args, **kwargs)

    def _lr22_code(body, *args, **kwargs):
        try:
            if isinstance(body, str) and _lr22_render_report(body):
                return
        except Exception:
            pass
        return _lr22_original_code(body, *args, **kwargs)

    st.markdown = _lr22_markdown
    st.write = _lr22_write
    st.code = _lr22_code
# === LOTTERY REPORTS BG VISUAL V22 END ===
