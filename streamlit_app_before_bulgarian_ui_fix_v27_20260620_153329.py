...
# === LOTTERY BULGARIAN UI POLISH V26 START ===
# Final Bulgarian UI polish layer.
# This affects only displayed text, not calculations, models, or datasets.
try:
    import pandas as _bg26_pd
    import inspect as _bg26_inspect
except Exception:  # pragma: no cover
    _bg26_pd = None
    _bg26_inspect = None


def _bg26_is_bulgarian() -> bool:
    try:
...
_BG26_EXACT = {
    # Remaining visible English model/card names
    ...
    # Technical model file/path labels displayed in cards
...
_BG26_REPLACE = [
    ...
    ("Cold + gap модел", "Студен + интервален модел"),
    ("Cold + Gap / интервален модел", "Студен + интервален модел"),
    ("Hot / честотен модел", "Горещ / честотен модел"),
    ("Middle / балансиран модел", "Среден / балансиран модел"),
    ("Gap / интервален модел", "Интервален модел"),
    ("Advanced ensemble", "Разширен ансамбъл"),
    ("Final combined", "Финален комбиниран модел"),
...
def _bg26_text(value):
    if not _bg26_is_bulgarian() or not isinstance(value, str):
        return value
    if value in _BG26_EXACT:
        return _BG26_EXACT[value]
    out = value
    for src, dst in _BG26_REPLACE:
        out = out.replace(src, dst)
    return out


def _bg26_column(col):
    raw = str(col)
    key = raw.strip().lower()
    if key in _BG26_COLUMN_MAP:
        return _BG26_COLUMN_MAP[key]
    return _bg26_text(raw)


def _bg26_dataframe(data):
    if not _bg26_is_bulgarian() or _bg26_pd is None:
        return data
    try:
        if not isinstance(data, _bg26_pd.DataFrame):
            return data
        df = data.copy()
        df = df.rename(columns={col: _bg26_column(col) for col in df.columns})
        for col in df.columns:
            if df[col].dtype == "object":
                df[col] = df[col].map(lambda x: _bg26_text(x) if isinstance(x, str) else x)
        return df
    except Exception:
        return data
...
def _bg26_css():
    if not _bg26_is_bulgarian():
        return
    st.markdown("""
<style>
/* Multiselect placeholder and file uploader polish where Streamlit allows CSS override. */
div[data-baseweb="select"] input::placeholder { color: transparent !important; }
div[data-baseweb="select"] input::before { content: "Избери числа"; }
...
if not getattr(st, "_lottery_bulgarian_ui_polish_v26", False):
    st._lottery_bulgarian_ui_polish_v26 = True

    _bg26_orig_markdown = st.markdown
    _bg26_orig_write = st.write
    _bg26_orig_caption = st.caption
    _bg26_orig_info = st.info
    _bg26_orig_success = st.success
    _bg26_orig_warning = st.warning
    _bg26_orig_error = st.error
    _bg26_orig_title = st.title
    _bg26_orig_header = st.header
    _bg26_orig_subheader = st.subheader
    _bg26_orig_metric = st.metric
    _bg26_orig_button = st.button
    _bg26_orig_checkbox = st.checkbox
    _bg26_orig_radio = st.radio
    _bg26_orig_selectbox = st.selectbox
    _bg26_orig_multiselect = st.multiselect
    _bg26_orig_text_input = st.text_input
    _bg26_orig_number_input = st.number_input
    _bg26_orig_file_uploader = st.file_uploader
    _bg26_orig_download_button = st.download_button
    _bg26_orig_dataframe = st.dataframe
    _bg26_orig_table = st.table

    def _bg26_markdown(body, *args, **kwargs):
        return _bg26_orig_markdown(_bg26_text(body), *args, **kwargs)

    def _bg26_write(*args, **kwargs):
        args = tuple(_bg26_text(a) if isinstance(a, str) else a for a in args)
        return _bg26_orig_write(*args, **kwargs)

    def _bg26_caption(body, *args, **kwargs):
        return _bg26_orig_caption(_bg26_text(body), *args, **kwargs)

    def _bg26_info(body, *args, **kwargs):
        return _bg26_orig_info(_bg26_text(body), *args, **kwargs)

    def _bg26_success(body, *args, **kwargs):
        return _bg26_orig_success(_bg26_text(body), *args, **kwargs)

    def _bg26_warning(body, *args, **kwargs):
        return _bg26_orig_warning(_bg26_text(body), *args, **kwargs)

    def _bg26_error(body, *args, **kwargs):
        return _bg26_orig_error(_bg26_text(body), *args, **kwargs)

    def _bg26_title(body, *args, **kwargs):
        return _bg26_orig_title(_bg26_text(body), *args, **kwargs)

    def _bg26_header(body, *args, **kwargs):
        return _bg26_orig_header(_bg26_text(body), *args, **kwargs)

    def _bg26_subheader(body, *args, **kwargs):
        return _bg26_orig_subheader(_bg26_text(body), *args, **kwargs)

    def _bg26_metric(label, value, delta=None, *args, **kwargs):
        return _bg26_orig_metric(_bg26_text(label), _bg26_text(value), _bg26_text(delta), *args, **kwargs)

    def _bg26_button(label, *args, **kwargs):
        return _bg26_orig_button(_bg26_text(label), *args, **kwargs)

    def _bg26_checkbox(label, *args, **kwargs):
        return _bg26_orig_checkbox(_bg26_text(label), *args, **kwargs)

    def _bg26_radio(label, options, *args, **kwargs):
        return _bg26_orig_radio(_bg26_text(label), options, *args, **kwargs)

    def _bg26_selectbox(label, options, *args, **kwargs):
        return _bg26_orig_selectbox(_bg26_text(label), options, *args, **kwargs)

    def _bg26_multiselect(label, options, *args, **kwargs):
        _bg26_css()
        if _bg26_is_bulgarian() and _bg26_inspect is not None:
            try:
                if "placeholder" in _bg26_inspect.signature(_bg26_orig_multiselect).parameters:
                    kwargs.setdefault("placeholder", "Избери числа")
            except Exception:
                pass
        return _bg26_orig_multiselect(_bg26_text(label), options, *args, **kwargs)

    def _bg26_text_input(label, *args, **kwargs):
        if "value" in kwargs and isinstance(kwargs.get("value"), str):
            kwargs["value"] = _bg26_text(kwargs["value"])
        if "placeholder" in kwargs and isinstance(kwargs.get("placeholder"), str):
            kwargs["placeholder"] = _bg26_text(kwargs["placeholder"])
        return _bg26_orig_text_input(_bg26_text(label), *args, **kwargs)

    def _bg26_number_input(label, *args, **kwargs):
        return _bg26_orig_number_input(_bg26_text(label), *args, **kwargs)

    def _bg26_file_uploader(label, *args, **kwargs):
        _bg26_css()
        return _bg26_orig_file_uploader(_bg26_text(label), *args, **kwargs)

    def _bg26_download_button(label, *args, **kwargs):
        return _bg26_orig_download_button(_bg26_text(label), *args, **kwargs)

    def _bg26_dataframe(data=None, *args, **kwargs):
        return _bg26_orig_dataframe(_bg26_dataframe(data), *args, **kwargs)

    def _bg26_table(data=None, *args, **kwargs):
        return _bg26_orig_table(_bg26_dataframe(data), *args, **kwargs)

    st.markdown = _bg26_markdown
    st.write = _bg26_write
    st.caption = _bg26_caption
    st.info = _bg26_info
    st.success = _bg26_success
    st.warning = _bg26_warning
    st.error = _bg26_error
    st.title = _bg26_title
    st.header = _bg26_header
    st.subheader = _bg26_subheader
    st.metric = _bg26_metric
    st.button = _bg26_button
    st.checkbox = _bg26_checkbox
    st.radio = _bg26_radio
    st.selectbox = _bg26_selectbox
    st.multiselect = _bg26_multiselect
    st.text_input = _bg26_text_input
    st.number_input = _bg26_number_input
    st.file_uploader = _bg26_file_uploader
    st.download_button = _bg26_download_button
    st.dataframe = _bg26_dataframe
    st.table = _bg26_table
# === LOTTERY BULGARIAN UI POLISH V26 END ===
