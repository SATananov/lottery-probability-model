...
# === LOTTERY BULGARIAN UI DEEP V25 START ===
# Stronger Bulgarian UI localization layer.
# It affects visual text only. It does not change model logic, datasets, or calculations.
try:
    import pandas as _bg25_pd
    import re as _bg25_re
except Exception:  # pragma: no cover
    _bg25_pd = None
    _bg25_re = None


def _bg25_is_bulgarian() -> bool:
    """Detect whether the current UI language is Bulgarian."""
    try:
...
_BG25_EXACT = {
    ...
    "Hot / честотен модел": "Горещ / честотен модел",
    "Cold + Gap": "Студен + gap модел",
    "Cold + Gap / interval model": "Студен + gap / интервален модел",
    "Cold + Gap / интервален модел": "Студен + gap / интервален модел",
    "Middle / Balanced": "Среден / балансиран модел",
    "Middle / балансиран модел": "Среден / балансиран модел",
    "Gap / Interval": "Gap / интервален модел",
    ...
    "Combined Recommendations": "Комбинирани препоръки",
    "Combined model": "Комбиниран модел",
    "Advanced Lab": "Разширена лаборатория",
    "Advanced laboratory": "Разширена лаборатория",
    ...
    "Back to main menu": "Назад към основното меню",
    ...
    "Automatically update models after save": "Автоматично обнови моделите след запис",
    ...
    "Matched numbers": "Съвпаднали числа",
    "Your ticket": "Твоят фиш",
    "Run simulation": "Пусни симулация",
    "Number of simulations": "Брой симулации",
    "Historical replay": "Исторически replay",
    ...
    "Advanced backtesting engine": "Разширен backtesting engine",
    ...
    "Preview lines": "Преглед на редове",
    ...
    "Average interval": "Среден интервал",
    "Draw": "Тираж",
    "Actual": "Реални числа",
    "Actual numbers": "Реални числа",
    "Advanced": "Разширен модел",
    "Advanced ticket": "Advanced фиш",
    "Advanced matches": "Advanced съвпадения",
    "Random": "Случаен фиш",
    "Random ticket": "Случаен фиш",
    "Random matches": "Random съвпадения",
    "Matches": "Съвпадения",
    "Strategy": "Стратегия",
    "Code": "Код",
    "Average": "Средно",
...
_BG25_COLUMN_MAP = {
    ...
    "z_score": "Z-score",
    "gap": "Gap",
    "gap / интервален": "Gap / интервал",
    "avg_interval": "Среден интервал",
    "category": "Категория",
    "times drawn": "Появявания",
    "empirical %": "Емпиричен %",
    "expected %": "Очакван %",
    "current gap": "Текущ gap",
    "average interval": "Среден интервал",
    "draw": "Тираж",
    "actual": "Реални числа",
    "actual numbers": "Реални числа",
...
def _bg25_translate_text(value):
    if not _bg25_is_bulgarian() or not isinstance(value, str):
        return value
    if value in _BG25_EXACT:
        return _BG25_EXACT[value]
    text_value = value
    for src, dst in _BG25_PHRASES:
        text_value = text_value.replace(src, dst)
    return text_value


def _bg25_translate_column(col):
    raw = str(col)
    key = raw.strip().lower()
    if key in _BG25_COLUMN_MAP:
        return _BG25_COLUMN_MAP[key]
    if raw in _BG25_EXACT:
        return _BG25_EXACT[raw]
    return _bg25_translate_text(raw)


def _bg25_translate_dataframe(data):
    if not _bg25_is_bulgarian() or _bg25_pd is None:
        return data
    try:
        if not isinstance(data, _bg25_pd.DataFrame):
            return data
        df = data.copy()
        df = df.rename(columns={col: _bg25_translate_column(col) for col in df.columns})
        for col in df.columns:
            if df[col].dtype == "object":
                df[col] = df[col].map(lambda x: _bg25_translate_text(x) if isinstance(x, str) else x)
        return df
    except Exception:
        return data
...
if not getattr(st, "_lottery_bulgarian_ui_deep_v25", False):
    st._lottery_bulgarian_ui_deep_v25 = True

    _bg25_original_markdown = st.markdown
    _bg25_original_write = st.write
    _bg25_original_caption = st.caption
    _bg25_original_info = st.info
    _bg25_original_success = st.success
    _bg25_original_warning = st.warning
    _bg25_original_error = st.error
    _bg25_original_title = st.title
    _bg25_original_header = st.header
    _bg25_original_subheader = st.subheader
    _bg25_original_metric = st.metric
    _bg25_original_button = st.button
    _bg25_original_checkbox = st.checkbox
    _bg25_original_radio = st.radio
    _bg25_original_selectbox = st.selectbox
    _bg25_original_multiselect = st.multiselect
    _bg25_original_text_input = st.text_input
    _bg25_original_number_input = st.number_input
    _bg25_original_file_uploader = st.file_uploader
    _bg25_original_download_button = st.download_button
    _bg25_original_dataframe = st.dataframe
    _bg25_original_table = st.table

    def _bg25_markdown(body, *args, **kwargs):
        return _bg25_original_markdown(_bg25_translate_text(body), *args, **kwargs)

    def _bg25_write(*args, **kwargs):
        args = tuple(_bg25_translate_text(a) if isinstance(a, str) else a for a in args)
        return _bg25_original_write(*args, **kwargs)

    def _bg25_caption(body, *args, **kwargs):
        return _bg25_original_caption(_bg25_translate_text(body), *args, **kwargs)

    def _bg25_info(body, *args, **kwargs):
        return _bg25_original_info(_bg25_translate_text(body), *args, **kwargs)

    def _bg25_success(body, *args, **kwargs):
        return _bg25_original_success(_bg25_translate_text(body), *args, **kwargs)

    def _bg25_warning(body, *args, **kwargs):
        return _bg25_original_warning(_bg25_translate_text(body), *args, **kwargs)

    def _bg25_error(body, *args, **kwargs):
        return _bg25_original_error(_bg25_translate_text(body), *args, **kwargs)

    def _bg25_title(body, *args, **kwargs):
        return _bg25_original_title(_bg25_translate_text(body), *args, **kwargs)

    def _bg25_header(body, *args, **kwargs):
        return _bg25_original_header(_bg25_translate_text(body), *args, **kwargs)

    def _bg25_subheader(body, *args, **kwargs):
        return _bg25_original_subheader(_bg25_translate_text(body), *args, **kwargs)

    def _bg25_metric(label, value, delta=None, *args, **kwargs):
        return _bg25_original_metric(_bg25_translate_text(label), _bg25_translate_text(value), _bg25_translate_text(delta), *args, **kwargs)

    def _bg25_button(label, *args, **kwargs):
        return _bg25_original_button(_bg25_translate_text(label), *args, **kwargs)

    def _bg25_checkbox(label, *args, **kwargs):
        return _bg25_original_checkbox(_bg25_translate_text(label), *args, **kwargs)

    def _bg25_radio(label, options, *args, **kwargs):
        return _bg25_original_radio(_bg25_translate_text(label), options, *args, **kwargs)

    def _bg25_selectbox(label, options, *args, **kwargs):
        return _bg25_original_selectbox(_bg25_translate_text(label), options, *args, **kwargs)

    def _bg25_multiselect(label, options, *args, **kwargs):
        return _bg25_original_multiselect(_bg25_translate_text(label), options, *args, **kwargs)

    def _bg25_text_input(label, *args, **kwargs):
        if "value" in kwargs and isinstance(kwargs.get("value"), str):
            kwargs["value"] = _bg25_translate_text(kwargs["value"])
        return _bg25_original_text_input(_bg25_translate_text(label), *args, **kwargs)

    def _bg25_number_input(label, *args, **kwargs):
        return _bg25_original_number_input(_bg25_translate_text(label), *args, **kwargs)

    def _bg25_file_uploader(label, *args, **kwargs):
        _bg25_css()
        return _bg25_original_file_uploader(_bg25_translate_text(label), *args, **kwargs)

    def _bg25_download_button(label, *args, **kwargs):
        return _bg25_original_download_button(_bg25_translate_text(label), *args, **kwargs)

    def _bg25_dataframe(data=None, *args, **kwargs):
        return _bg25_original_dataframe(_bg25_translate_dataframe(data), *args, **kwargs)

    def _bg25_table(data=None, *args, **kwargs):
        return _bg25_original_table(_bg25_translate_dataframe(data), *args, **kwargs)

    st.markdown = _bg25_markdown
    st.write = _bg25_write
    st.caption = _bg25_caption
    st.info = _bg25_info
    st.success = _bg25_success
    st.warning = _bg25_warning
    st.error = _bg25_error
    st.title = _bg25_title
    st.header = _bg25_header
    st.subheader = _bg25_subheader
    st.metric = _bg25_metric
    st.button = _bg25_button
    st.checkbox = _bg25_checkbox
    st.radio = _bg25_radio
    st.selectbox = _bg25_selectbox
    st.multiselect = _bg25_multiselect
    st.text_input = _bg25_text_input
    st.number_input = _bg25_number_input
    st.file_uploader = _bg25_file_uploader
    st.download_button = _bg25_download_button
    st.dataframe = _bg25_dataframe
    st.table = _bg25_table
# === LOTTERY BULGARIAN UI DEEP V25 END ===
