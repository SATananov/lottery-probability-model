...
Step 151.2 commits the local numeric selectbox repair, closes the two discovered Streamlit runtime failures and makes the repository reproducible across Windows and fresh clones. All tracked text is canonical LF, release manifests are generated from those canonical bytes, and the active Step 148 lock is validated dynamically from the current ledger rather than from a hardcoded old lock.

The Training Center now preserves real Bulgarian Unicode and forces child Python output to UTF-8. Step 94 uses unique download keys for the real and demo result tabs. Step 80 follows the current official dataset instead of frozen historical constants.

Final acceptance requires a clean `main...origin/main` state and a separate clone passing `python scripts/verify_step_151_2.py --require-synced`.


## Step 151.2.1 — User-Facing Backup UI & Technical Details Separation Closure

Step 151.2.1 довършва правилото от Step 150.2 за разделяне на нормалния потребителски изглед от техническата диагностика. Страницата за чист архив вече се представя като **„Резервно копие на приложението“**, обяснява какво ще получи потребителят и защо първо трябва да има синхронизация с GitHub.
