...
def _guide_sections() -> list[dict[str, Any]]:
    return [
        ...
        {"section_order": 3, "section": "Добавяне на нов тираж", "purpose": "Въвежда нови реални числа и синхронизира dataset-ите и зависимите модели.", "user_action": "Първо провери числата внимателно, после пускай обновяването.", "safe_note": SAFE_NOTE},
        ...
        {"section_order": 9, "section": "Системни проверки", "purpose": "Проверяват sync chain, release readiness, UX, manifest, dataset consistency и техническа чистота.", "user_action": "Пускай ги след важни промени, особено след добавяне на тираж.", "safe_note": SAFE_NOTE},
...
def _workflow_rows() -> list[dict[str, Any]]:
    return [
        {"step_order": 1, "page": "Добавяне на тираж", "when_to_use": "Когато има нов официален тираж.", "expected_result": "Dataset-ите и моделните артефакти се обновяват.", "caution": "Въведи числата точно. Bonus не се смесва с основните 6 числа."},
        {"step_order": 2, "page": "Анализ на нов тираж", "when_to_use": "След въвеждане или преди сравнение с предишни пакети.", "expected_result": "Виждаш как последният тираж се отнася към исторически сигнали.", "caution": "Това е post-draw анализ, не обещание за бъдещ резултат."},
        ...
        {"step_order": 5, "page": "Умен генератор с тегла", "when_to_use": "Когато подготвяш кандидат комбинации.", "expected_result": "Получаваш структурирани кандидати с weighted score.", "caution": "Не генерирай прекалено голям пакет без лимит."},
        ...
        {"step_order": 9, "page": "Експорт и изпълнение", "when_to_use": "Когато трябва да копираш или отпечаташ пакета.", "expected_result": "Имаш чист execution checklist и copy text.", "caution": "Провери ръчно всяко число преди използване."},
        {"step_order": 10, "page": "Финален release пакет / Контрол на синхрона", "when_to_use": "След по-големи промени или преди clean ZIP.", "expected_result": "Проверяваш дали app/reports/models са синхронизирани.", "caution": "Не прави clean ZIP при dirty git status или неуспешни проверки."},
    ]
...
def _safe_checklist_rows() -> list[dict[str, Any]]:
    return [
        ...
        {"check_order": 6, "check_item": "След нов тираж се пуска синхрон", "recommended_behavior": "След Add Draw използвай refresh/sync chain според приложението.", "risk_if_ignored": "Остарели модели и отчети."},
...
def build_final_user_manual_center() -> dict[str, Any]:
    ...
    md_lines = [
        ...
        f"- Guide секции: **{len(guide_sections)}**",
        f"- Workflow стъпки: **{len(workflow_rows)}**",
        f"- Safe checklist точки: **{len(safe_checklist)}**",
        "",
