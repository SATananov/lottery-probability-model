...
# === LOTTERY BULGARIAN UI GLOBAL V24 START ===
# Runtime Bulgarian UI localization layer.
try:
    import pandas as _bg24_pd
except Exception:  # pragma: no cover
    _bg24_pd = None


def _bg24_is_bulgarian() -> bool:
    try:
...
_BG24_EXACT = {
    ...
    "Best average-match strategy in this backtest": "Най-добрата стратегия по средни съвпадения в този backtest",
    "Treat this as a model check, not proof that future lottery draws are predictable.": "Това е проверка на модела, не доказателство, че бъдещите тегления са предсказуеми.",
    "This is a model check, not proof that future lottery draws are predictable.": "Това е проверка на модела, не доказателство, че бъдещите тегления са предсказуеми.",
...
_BG24_CONTAINS = [
    ("200MB per file", "до 200MB на файл"),
    ("matches", "съвпадения"),
    ("match", "съвпадение"),
    ("actual=", "реални числа="),
...
def _bg24_translate_text(value):
    if not _bg24_is_bulgarian() or not isinstance(value, str):
        return value
    if value in _BG24_EXACT:
        return _BG24_EXACT[value]
    text_value = value
    for src, dst in _BG24_CONTAINS:
        text_value = text_value.replace(src, dst)
    for src, dst in _BG24_STRATEGIES.items():
        text_value = text_value.replace(src, dst)
    return text_value


def _bg24_translate_dataframe(data):
    if not _bg24_is_bulgarian() or _bg24_pd is None:
        return data
    try:
        if isinstance(data, _bg24_pd.DataFrame):
            df = data.copy()
        else:
            return data
        df = df.rename(columns={col: _bg24_translate_text(str(col)) for col in df.columns})
        for col in df.columns:
            if df[col].dtype == "object":
                df[col] = df[col].map(lambda x: _bg24_translate_text(x) if isinstance(x, str) else x)
        return df
    except Exception:
        return data
...
if not getattr(st, "_lottery_bulgarian_ui_global_v24", False):
    st._lottery_bulgarian_ui_global_v24 = True

    _bg24_original_markdown = st.markdown
    _bg24_original_write = st.write
    _bg24_original_caption = st.caption
    _bg24_original_info = st.info
    _bg24_original_success = st.success
    _bg24_original_warning = st.warning
    _bg24_original_error = st.error
    _bg24_original_title = st.title
    _bg24_original_header = st.header
    _bg24_original_subheader = st.subheader
    _bg24_original_metric = st.metric
    _bg24_original_button = st.button
    _bg24_original_checkbox = st.checkbox
    _bg24_original_radio = st.radio
    _bg24_original_selectbox = st.selectbox
    _bg24_original_multiselect = st.multiselect
    _bg24_original_text_input = st.text_input
    _bg24_original_number_input = st.number_input
    _bg24_original_file_uploader = st.file_uploader
    _bg24_original_download_button = st.download_button
    _bg24_original_dataframe = st.dataframe
    _bg24_original_table = st.table

    def _bg24_markdown(body, *args, **kwargs):
        return _bg24_original_markdown(_bg24_translate_text(body), *args, **kwargs)

    def _bg24_write(*args, **kwargs):
        args = tuple(_bg24_translate_text(a) if isinstance(a, str) else a for a in args)
        return _bg24_original_write(*args, **kwargs)

    def _bg24_caption(body, *args, **kwargs):
        return _bg24_original_caption(_bg24_translate_text(body), *args, **kwargs)

    def _bg24_info(body, *args, **kwargs):
        return _bg24_original_info(_bg24_translate_text(body), *args, **kwargs)

    def _bg24_success(body, *args, **kwargs):
        return _bg24_original_success(_bg24_translate_text(body), *args, **kwargs)

    def _bg24_warning(body, *args, **kwargs):
        return _bg24_original_warning(_bg24_translate_text(body), *args, **kwargs)

    def _bg24_error(body, *args, **kwargs):
        return _bg24_original_error(_bg24_translate_text(body), *args, **kwargs)

    def _bg24_title(body, *args, **kwargs):
        return _bg24_original_title(_bg24_translate_text(body), *args, **kwargs)

    def _bg24_header(body, *args, **kwargs):
        return _bg24_original_header(_bg24_translate_text(body), *args, **kwargs)

    def _bg24_subheader(body, *args, **kwargs):
        return _bg24_original_subheader(_bg24_translate_text(body), *args, **kwargs)

    def _bg24_metric(label, value, delta=None, *args, **kwargs):
        return _bg24_original_metric(_bg24_translate_text(label), _bg24_translate_text(value), _bg24_translate_text(delta), *args, **kwargs)

    def _bg24_button(label, *args, **kwargs):
        return _bg24_original_button(_bg24_translate_text(label), *args, **kwargs)

    def _bg24_checkbox(label, *args, **kwargs):
        return _bg24_original_checkbox(_bg24_translate_text(label), *args, **kwargs)

    def _bg24_radio(label, options, *args, **kwargs):
        return _bg24_original_radio(_bg24_translate_text(label), options, *args, **kwargs)

    def _bg24_selectbox(label, options, *args, **kwargs):
        return _bg24_original_selectbox(_bg24_translate_text(label), options, *args, **kwargs)

    def _bg24_multiselect(label, options, *args, **kwargs):
        return _bg24_original_multiselect(_bg24_translate_text(label), options, *args, **kwargs)

    def _bg24_text_input(label, *args, **kwargs):
        if "value" in kwargs and isinstance(kwargs.get("value"), str):
            kwargs["value"] = _bg24_translate_text(kwargs["value"])
        return _bg24_original_text_input(_bg24_translate_text(label), *args, **kwargs)

    def _bg24_number_input(label, *args, **kwargs):
        return _bg24_original_number_input(_bg24_translate_text(label), *args, **kwargs)

    def _bg24_file_uploader(label, *args, **kwargs):
        _bg24_css()
        return _bg24_original_file_uploader(_bg24_translate_text(label), *args, **kwargs)

    def _bg24_download_button(label, *args, **kwargs):
        return _bg24_original_download_button(_bg24_translate_text(label), *args, **kwargs)

    def _bg24_dataframe(data=None, *args, **kwargs):
        return _bg24_original_dataframe(_bg24_translate_dataframe(data), *args, **kwargs)

    def _bg24_table(data=None, *args, **kwargs):
        return _bg24_original_table(_bg24_translate_dataframe(data), *args, **kwargs)

    st.markdown = _bg24_markdown
    st.write = _bg24_write
    st.caption = _bg24_caption
    st.info = _bg24_info
    st.success = _bg24_success
    st.warning = _bg24_warning
    st.error = _bg24_error
    st.title = _bg24_title
    st.header = _bg24_header
    st.subheader = _bg24_subheader
    st.metric = _bg24_metric
    st.button = _bg24_button
    st.checkbox = _bg24_checkbox
    st.radio = _bg24_radio
    st.selectbox = _bg24_selectbox
    st.multiselect = _bg24_multiselect
    st.text_input = _bg24_text_input
    st.number_input = _bg24_number_input
    st.file_uploader = _bg24_file_uploader
    st.download_button = _bg24_download_button
    st.dataframe = _bg24_dataframe
    st.table = _bg24_table
# === LOTTERY BULGARIAN UI GLOBAL V24 END ===
