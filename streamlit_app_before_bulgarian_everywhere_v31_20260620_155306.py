...
# === LOTTERY BULGARIAN FULL UI V30 START ===
# Bulgarian UI final layer: text-only localization.
# It does not change model logic, datasets, formulas, or generated files.
try:
    import pandas as _bg30_pd
    import inspect as _bg30_inspect
except Exception:  # pragma: no cover
    _bg30_pd = None
    _bg30_inspect = None


def _bg30_is_bulgarian() -> bool:
    try:
...
def _bg30_text(value):
    if not _bg30_is_bulgarian() or not isinstance(value, str):
        return value
    if value in _BG30_TEXT:
        return _BG30_TEXT[value]
    out = value
    for src, dst in _BG30_REPLACE:
        out = out.replace(src, dst)
    return out


def _bg30_translate_dataframe(data):
    if not _bg30_is_bulgarian() or _bg30_pd is None:
        return data
    try:
        if not isinstance(data, _bg30_pd.DataFrame):
            return data
        df = data.copy()
        df = df.rename(columns={col: _bg30_text(str(col)) for col in df.columns})
        for col in df.columns:
            if df[col].dtype == "object":
                df[col] = df[col].map(lambda x: _bg30_text(x) if isinstance(x, str) else x)
        return df
    except Exception:
        return data
...
if not getattr(st, "_lottery_bulgarian_full_ui_v30", False):
    st._lottery_bulgarian_full_ui_v30 = True

    _bg30_orig_markdown = st.markdown
    _bg30_orig_write = st.write
    _bg30_orig_caption = st.caption
    _bg30_orig_info = st.info
    _bg30_orig_success = st.success
    _bg30_orig_warning = st.warning
    _bg30_orig_error = st.error
    _bg30_orig_title = st.title
    _bg30_orig_header = st.header
    _bg30_orig_subheader = st.subheader
    _bg30_orig_metric = st.metric
    _bg30_orig_button = st.button
    _bg30_orig_checkbox = st.checkbox
    _bg30_orig_radio = st.radio
    _bg30_orig_tabs = st.tabs
    _bg30_orig_selectbox = st.selectbox
    _bg30_orig_slider = st.slider
    _bg30_orig_select_slider = st.select_slider
    _bg30_orig_multiselect = st.multiselect
    _bg30_orig_text_input = st.text_input
    _bg30_orig_number_input = st.number_input
    _bg30_orig_file_uploader = st.file_uploader
    _bg30_orig_download_button = st.download_button
    _bg30_orig_dataframe = st.dataframe
    _bg30_orig_table = st.table

    def _bg30_markdown(body, *args, **kwargs):
        return _bg30_orig_markdown(_bg30_text(body), *args, **kwargs)

    def _bg30_write(*args, **kwargs):
        args = tuple(_bg30_text(a) if isinstance(a, str) else a for a in args)
        return _bg30_orig_write(*args, **kwargs)

    def _bg30_caption(body, *args, **kwargs):
        return _bg30_orig_caption(_bg30_text(body), *args, **kwargs)

    def _bg30_info(body, *args, **kwargs):
        return _bg30_orig_info(_bg30_text(body), *args, **kwargs)

    def _bg30_success(body, *args, **kwargs):
        return _bg30_orig_success(_bg30_text(body), *args, **kwargs)

    def _bg30_warning(body, *args, **kwargs):
        return _bg30_orig_warning(_bg30_text(body), *args, **kwargs)

    def _bg30_error(body, *args, **kwargs):
        return _bg30_orig_error(_bg30_text(body), *args, **kwargs)

    def _bg30_title(body, *args, **kwargs):
        return _bg30_orig_title(_bg30_text(body), *args, **kwargs)

    def _bg30_header(body, *args, **kwargs):
        return _bg30_orig_header(_bg30_text(body), *args, **kwargs)

    def _bg30_subheader(body, *args, **kwargs):
        return _bg30_orig_subheader(_bg30_text(body), *args, **kwargs)

    def _bg30_metric(label, value, delta=None, *args, **kwargs):
        return _bg30_orig_metric(_bg30_text(label), _bg30_text(value), _bg30_text(delta), *args, **kwargs)

    def _bg30_button(label, *args, **kwargs):
        return _bg30_orig_button(_bg30_text(label), *args, **kwargs)

    def _bg30_checkbox(label, *args, **kwargs):
        return _bg30_orig_checkbox(_bg30_text(label), *args, **kwargs)

    def _bg30_radio(label, options, *args, **kwargs):
        translated_options = [_bg30_text(o) if isinstance(o, str) and o not in ("Български", "English") else o for o in options]
        return _bg30_orig_radio(_bg30_text(label), translated_options, *args, **kwargs)

    def _bg30_tabs(tabs, *args, **kwargs):
        return _bg30_orig_tabs([_bg30_text(t) if isinstance(t, str) else t for t in tabs], *args, **kwargs)

    def _bg30_selectbox(label, options, *args, **kwargs):
        translated_options = [_bg30_text(o) if isinstance(o, str) else o for o in options]
        return _bg30_orig_selectbox(_bg30_text(label), translated_options, *args, **kwargs)

    def _bg30_slider(label, *args, **kwargs):
        return _bg30_orig_slider(_bg30_text(label), *args, **kwargs)

    def _bg30_select_slider(label, *args, **kwargs):
        return _bg30_orig_select_slider(_bg30_text(label), *args, **kwargs)

    def _bg30_multiselect(label, options, *args, **kwargs):
        _bg30_css()
        if _bg30_is_bulgarian() and _bg30_inspect is not None:
            try:
                if "placeholder" in _bg30_inspect.signature(_bg30_orig_multiselect).parameters:
                    kwargs.setdefault("placeholder", "Избери числа")
            except Exception:
                pass
        return _bg30_orig_multiselect(_bg30_text(label), options, *args, **kwargs)

    def _bg30_text_input(label, *args, **kwargs):
        if "value" in kwargs and isinstance(kwargs.get("value"), str):
            kwargs["value"] = _bg30_text(kwargs["value"])
        if "placeholder" in kwargs and isinstance(kwargs.get("placeholder"), str):
            kwargs["placeholder"] = _bg30_text(kwargs["placeholder"])
        return _bg30_orig_text_input(_bg30_text(label), *args, **kwargs)

    def _bg30_number_input(label, *args, **kwargs):
        return _bg30_orig_number_input(_bg30_text(label), *args, **kwargs)

    def _bg30_file_uploader(label, *args, **kwargs):
        _bg30_css()
        return _bg30_orig_file_uploader(_bg30_text(label), *args, **kwargs)

    def _bg30_download_button(label, *args, **kwargs):
        return _bg30_orig_download_button(_bg30_text(label), *args, **kwargs)

    def _bg30_dataframe_widget(data=None, *args, **kwargs):
        return _bg30_orig_dataframe(_bg30_translate_dataframe(data), *args, **kwargs)

    def _bg30_table_widget(data=None, *args, **kwargs):
        return _bg30_orig_table(_bg30_translate_dataframe(data), *args, **kwargs)

    st.markdown = _bg30_markdown
    st.write = _bg30_write
    st.caption = _bg30_caption
    st.info = _bg30_info
    st.success = _bg30_success
    st.warning = _bg30_warning
    st.error = _bg30_error
    st.title = _bg30_title
    st.header = _bg30_header
    st.subheader = _bg30_subheader
    st.metric = _bg30_metric
    st.button = _bg30_button
    st.checkbox = _bg30_checkbox
    st.radio = _bg30_radio
    st.tabs = _bg30_tabs
    st.selectbox = _bg30_selectbox
    st.slider = _bg30_slider
    st.select_slider = _bg30_select_slider
    st.multiselect = _bg30_multiselect
    st.text_input = _bg30_text_input
    st.number_input = _bg30_number_input
    st.file_uploader = _bg30_file_uploader
    st.download_button = _bg30_download_button
    st.dataframe = _bg30_dataframe_widget
    st.table = _bg30_table_widget
# === LOTTERY BULGARIAN FULL UI V30 END ===
